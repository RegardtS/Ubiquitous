...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter implements GoogleApiClient.ConnectionCallbacks,
    ...
    public void onPerformSync(Account account, Bundle extras, String authority, ContentProviderClient provider, SyncResult syncResult) {
        ...
        // BEGIN wear build
        Log.wtf(LOG_TAG, "mGoogleApiClient created");
        ...
        try {
            ...
            Uri builtUri = Uri.parse(FORECAST_BASE_URL).buildUpon()
                    ...
                    .appendQueryParameter(APPID_PARAM, "e62e114b1d73a3839f3f3f7a34249f85")
                    .build();
    ...
    private void getWeatherDataFromJson(String forecastJsonStr,
        ...
        try {
            ...
            for (int i = 0; i < weatherArray.length(); i++) {
                ...
                if (i == 0) {
                    highTemp = Utility.formatTemperature(getContext(), high);
                    lowTemp = Utility.formatTemperature(getContext(), low);
                    sWeatherId = weatherId;
                    new AsyncTask<Void, Void, Void>() {
                        @Override
                        protected Void doInBackground(final Void... params) {
                            onStartWearableSync();
                            dataItemGenerate(PATH_CURRENT_TEMP);
                            return null;
                        }
            ...
            Log.wtf(LOG_TAG, "Sync Complete. " + cVVector.size() + " Inserted");
    ...
    private void dataItemGenerate(String path) {
        ...
        if (path.equals(PATH_TIME)) {
            putDataMapRequest.getDataMap().putString(KEY_TIME, Long.toString(System.currentTimeMillis()));
        }
        putDataMapRequest.getDataMap().putString(KEY_LOW, lowTemp);
        putDataMapRequest.getDataMap().putAsset(KEY_IMAGE, Utility.toAsset(getContext(), sWeatherId));
    ...
    private void sendStartWeatherMessage(String node) {
        Wearable.MessageApi.sendMessage(
                mGoogleApiClient, node, PATH_START_WEATHER_SYNC, new byte[0]).setResultCallback(
                new ResultCallback<MessageApi.SendMessageResult>() {
                    @Override
                    public void onResult(MessageApi.SendMessageResult sendMessageResult) {
                        if (!sendMessageResult.getStatus().isSuccess()) {
                            Log.wtf("test", sendMessageResult.getStatus().getStatusCode() + "");
                        }
                    }
                }
        );
